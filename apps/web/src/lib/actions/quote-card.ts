...
import { quoteCardSchema } from "@/lib/validations/quote-card";
...
export async function saveQuoteCardAction(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  const parsed = quoteCardSchema.safeParse({
    itemId: formData.get("itemId"),
    title: formData.get("title"),
    content: formData.get("content"),
    align: formData.get("align"),
    highlightSubstring: formData.get("highlightSubstring"),
  ...
  const doc = buildQuoteCardDocument({
    content: parsed.data.content,
    align: parsed.data.align,
    highlightSubstring: parsed.data.highlightSubstring || undefined,
  ...
  if (parsed.data.itemId) {
    ...
    revalidatePath("/posts/frase-de-efeito");
    redirect(`/posts/frase-de-efeito?id=${existing.id}&saved=1`);
  }
  ...
  revalidatePath("/posts/frase-de-efeito");
  redirect(`/posts/frase-de-efeito?id=${created.id}&saved=1`);
}

export async function renderQuoteCardAction(itemId: string): Promise<ActionResult & { downloadUrl?: string }> {
  const context = await getWorkspaceContext();
  if (!context?.workspace) return { error: "Sessão expirada." };

  const preflight = costPreflight({ providerId: "local-og-image", operation: "quote_card_render" });
